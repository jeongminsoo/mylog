...
SELECT MAX(HNO) FROM HASHTAG WHERE HNAME='공부';
    ...
    FROM(SELECT ROWNUM RN, A.*
        FROM(SELECT R.*, MNAME,(SELECT COUNT(*)FROM RPBOARD B WHERE B.RNUM= R.RNUM)AS cnt FROM RBOARD R, MEMBER M WHERE M.MID = R.MID AND RTITLE LIKE '%'||''||'%' ORDER BY RRDATE DESC)A)
    WHERE RN BETWEEN 1 AND 30;
