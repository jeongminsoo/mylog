...
SELECT B.*, TDURING, T.TNAME, TTODAY FROM TTABLE B, TIMER T WHERE B.TNO=T.TNO AND B.MID='aaa' AND TO_CHAR(TTODAY, 'yy/mm/dd')=TO_CHAR(SYSDATE-1, 'yy/mm/dd');
SELECT B.*, TDURING, T.TNAME, TTODAY FROM TTABLE B, TIMER T WHERE B.TNO=T.TNO AND B.MID='aaa' AND TO_CHAR(TTODAY, 'yy/mm/dd')=TO_CHAR(SYSDATE+1, 'yy/mm/dd');
...
select * from timer;
SELECT A.* 
    FROM(SELECT ROWNUM RN, r.*, MNAME, (SELECT COUNT(*)FROM RPBOARD B WHERE B.RNUM= R.RNUM)AS RCNT FROM RBOARD R, MEMBER M, FRIEND F,(SELECT RBOARD.* FROM BOARD_TAG JOIN RBOARD ON RBOARD.RNUM=BOARD_TAG.RNUM
WHERE BOARD_TAG.HNO =2)WHERE(M.MID = R.MID AND F.MID = M.MID AND MSTATUS =1 AND RSTATUS=0) 
