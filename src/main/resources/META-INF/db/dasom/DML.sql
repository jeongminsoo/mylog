...
--내가 볼 수 있는 감상문(제목 검색한 경우)
	SELECT B.* FROM(
    		SELECT ROWNUM RN, A.* FROM(SELECT DISTINCT RSTATUS, RNUM,RTITLE, MNAME, RRDATE,RFILENAME,(SELECT COUNT(*)FROM RPBOARD B WHERE B.RNUM= R.RNUM)AS RCNT FROM RBOARD R, MEMBER M, FRIEND F
                WHERE(R.MID = F.MID AND M.MID = F.MID AND MSTATUS =1 AND RSTATUS=0 AND RTITLE LIKE '%'||'빛'||'%') 
                 OR (R.MID=F.FID AND M.MID=F.FID AND MSTATUS =1  AND F.MID='aaa' AND RSTATUS = 1 AND RTITLE LIKE '%'||'빛'||'%')
                  OR (R.MID=M.MID AND M.MID=F.MID AND M.MID='aaa' AND RSTATUS = 2 AND RTITLE LIKE '%'||'빛'||'%' ) ORDER BY RRDATE DESC )A)B
			WHERE RN BETWEEN 1 AND 30;


   SELECT B.* FROM(
    SELECT ROWNUM RN, A.* FROM(SELECT DISTINCT RSTATUS, RNUM,RTITLE, MNAME, RRDATE,RFILENAME,(SELECT COUNT(*)FROM RPBOARD B WHERE B.RNUM= R.RNUM)AS RCNT FROM RBOARD R, MEMBER M, FRIEND F
                WHERE(M.MID = R.MID AND F.MID = R.MID AND M.MID='aaa' AND MSTATUS =1 AND RSTATUS=0 AND RTITLE LIKE '%'||'빛'||'%') 
                 OR (M.MID=F.FID AND R.MID=F.FID AND MSTATUS =1  AND F.MID='aaa' AND RSTATUS = 1 AND RTITLE LIKE '%'||'빛'||'%')
                  OR (F.FID=R.MID AND F.MID=M.MID AND M.MID='aaa' AND RSTATUS = 2 AND RTITLE LIKE '%'||'빛'||'%')ORDER BY RRDATE DESC )A)B
			WHERE RN BETWEEN 1 AND 100;
